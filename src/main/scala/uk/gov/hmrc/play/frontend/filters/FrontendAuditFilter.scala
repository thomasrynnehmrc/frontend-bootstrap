...
import uk.gov.hmrc.play.frontend.config.{EventTypes, HttpAuditEvent}
      ...
      def performAudit(requestBody: String, maybeResult: Try[Result])(responseBody: String): Unit =
        maybeResult match {
          case Success(result) =>
            ...
            auditConnector.sendEvent(dataEvent(EventTypes.RequestReceived, requestHeader.uri, requestHeader, detail))
          case Failure(f) =>
            auditConnector.sendEvent(
              dataEvent(
                EventTypes.RequestReceived,
                requestHeader.uri,
                requestHeader,
...
private class RequestBodyCaptor(val loggingContext: String, val maxBodyLength: Int, callback: (ByteString) => Unit)
