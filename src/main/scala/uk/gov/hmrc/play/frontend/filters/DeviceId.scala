...
object DeviceId {
  ...
  def generateSignature(deviceId: DeviceId) =
    deviceId.timestamp.fold(s"${deviceId.uuid}$Token2${deviceId.hash}")(time =>
      s"$MdtpDeviceId$Token1${deviceId.uuid}$Token1$time$Token2${deviceId.hash}")
  ...
  def deviceIdHashIsValid(
    hash: String,
    uuid: String,
    timestamp: Option[Long],
    secret: String,
    previousSecrets: Seq[String]) = {
  ...
  def from(value: String, secret: String, previousSecrets: Seq[String]) = {
    ...
    def isValid(prefix: String, uuid: String, timestamp: String, hash: String) =
      isValidPrefix(prefix) && validUuid(uuid) && validLongTime(timestamp) && deviceIdHashIsValid(
        hash,
        uuid,
        Some(timestamp.toLong),
        secret,
        previousSecrets)

    def isValidLegacy(uuid: String, hash: String) =
      validUuid(uuid) && deviceIdHashIsValid(hash, uuid, None, secret, previousSecrets)
