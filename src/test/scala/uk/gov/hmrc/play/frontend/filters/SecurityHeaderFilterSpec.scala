...
class SecurityHeaderFilterSpec extends WordSpecLike with Matchers with MockitoSugar with ScalaFutures {
  ...
  val appConfigDecodingEnabled: Map[String, _] = Map(
    "security.headers.filter.decoding.enabled"   -> true,
    "play.filters.headers.contentSecurityPolicy" -> "ZGVmYXVsdC1zcmMgJ3NlbGYn")
  ...
  "SecurityHeaderFilter" should {
    ...
    "add security header to an http response with filter enabled and  settings decoding disabled" in new WithApplication(
      appDecodingDisabled) with Setup {
    ...
    "add security header to an http response with filter enabled and  settings decoding enabled" in new WithApplication(
      appDecodingEnabled) with Setup {
