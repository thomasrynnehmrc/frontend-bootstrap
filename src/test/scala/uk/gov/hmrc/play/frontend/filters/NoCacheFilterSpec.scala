...
class NoCacheFilterSpec extends WordSpecLike with Matchers with MockitoSugar with ScalaFutures with OneAppPerTest {
  ...
  "During result post-processing, the filter" should {
    ...
    "override any existing cache-control header with no-cache" in new Setup {
      getResult(HeaderNames.CACHE_CONTROL -> "max-age=300") should be(Ok.withHeaders(CommonHeaders.NoCacheHeader))
    }
